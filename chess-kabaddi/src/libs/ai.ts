...
function bestKingMove(state: GameState): Pos | null {
  ...
  for (const m of legal) {
    ...
    const cornerBonus =
      (m.x === 0 || m.x === state.size - 1) &&
      (m.y === 0 || m.y === state.size - 1)
        ? 0.5
        : 0;
...
export function defenderAIMove(
  state: GameState
): { piece: Piece; to: Pos } | null {
  ...
  for (const w of whites) {
    ...
    for (const to of legal) {
      ...
      const target = state.pieces.find(
        (p) => p.pos.x === to.x && p.pos.y === to.y
      );
      if (target && target.color === "black")
        score += (VALUE[target.type] || 1) * 5;
